...
try {
    ...
    foreach ($records as $index => $record) {
        $customer = null;
        try {
            $MeterNo =  'MTR-' . $record['MeterNo'];
            // Generate a random password
            $password = substr(str_shuffle('12345678'), 0, 8);

            $balance = (float)$record['Tbal'];
            $name = $record['NamePrimary'];
            $primaryphone = $record['NumberPrimary'];
            $location = isset($record['Area']) ? $record['Area'] : '-';
            $date = $record['Date'];
            try {
                ...
                if (User::where('telephone', $record['NumberPrimary'])->exists()) {
                    $updated++;
                    $errors[] = "Row {$index}: Telephone number {$record['NumberPrimary']} already exists";
                    $customer = User::where('telephone', $record['NumberPrimary'])->first();
                    $used[] = "Row {$index}: Customer {$customer->id}->{$customer->name} Found";
                } else {
                    //we create
                    $customer = User::create([
                        'name' => $name,
                        'email' => $primaryphone . '@placeholder.com', // Temporary email
                ...
                if (isset($record['NumberSecondary']) && isset($record['NameSecondary'])) {
                    //check if it exists in contacts
                    if (!Contact::where('phone', $record['NumberSecondary'])
                        ->where('user_id', $customer->id)
                        ->where('tenant_id', $tenantId)
                        ->exists())
                        $contactName = isset($record['NameSecondary']) ? $record['NameSecondary'] : '-';
                    Contact::create([
                        'user_id' => $customer->id,
                        'tenant_id' => $tenantId,
                        'name' => $contactName,
                        'phone' => $record['NumberSecondary'],
                        'created_at' => $parsedDate,
                        'updated_at' => $parsedDate,
                    ]);
                }
                ...
                if (!empty($MeterNo)) {
                    ...
                    $meterAssignment = MeterAssignment::create([
                        ...
                        'connection_fee_paid' => true,
                        'assignment_date' =>  $parsedDate,
                        'initial_reading' => 0.00,
                        'is_active' => true,
                        'tenant_id' => $tenantId,
                        'notes' => 'Assigned ' . $MeterNo . ' to ' . $customer->name . ' during import',
                        'created_at' => $parsedDate,
                        'updated_at' => $parsedDate,
                    ]);
                ...
                DB::commit();
                $imported++;
                echo "Imported customer: {$name} (Row {$index})\n";
            } catch (\Exception $e) {
                DB::rollBack();
